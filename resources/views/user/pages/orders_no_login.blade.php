                ...
                <div class="col-lg-12">
                    <div class="shoping__cart__table">
                        ...
                        <table class="table table-striped table-bordered nowrap">
                            ...
                            <tbody>
                            <?php
                                // Khởi tạo một mảng rỗng để chứa các giá trị id
                                $orderIds = [];

                                // Lặp qua mảng $orders để lấy ra tất cả các giá trị id
                                foreach($orders as $value) {
                                    $orderIds[] = $value['id'];
                                }

                                // Lấy ra giá trị lớn nhất trong mảng $orderIds
                                $maxOrderId = max($orderIds);

                                // Dùng giá trị lớn nhất này để thay thế cho '16'
                                ?>
                                @foreach($orders as $value)
                                @if($value['users_id']==2 && $value['id']==$maxOrderId)
                                <tr>
                                    <td>{!! $value['id'] !!}</td>
